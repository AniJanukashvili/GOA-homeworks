#1

number1 = (input("type any number: "))
number2 = (input("type another number: "))


print(number1==number2)


# 2

print(10 > 5)
print(10 > 7)
print(10 > 13)
print(10 > 8)
print(10 > 17)


# 3

print(10 >= 14)
print(10 >= 5)
print(10 >= 2)
print(10 >= 1)
print(10 >= 19)

# 4

print(10 < 5)
print(10 < 7)
print(10 < 13)
print(10 < 8)
print(10 < 17)

# 5

print(10 <= 14)
print(10 <= 5)
print(10 <= 2)
print(10 <= 1)
print(10 <= 19)

# #6

print(5 == 5)
print(6 == 6)
print(2 == 2)
print(1 == 1)
print(17 == 17)

# # 7

print(9 != 8)
print(5 != 4)
print(10 != 17)
print(14 != 7)
print(4 != 7)

# # 8

number1 = (input("type any number: "))
number2 = (input("type another number: "))

print(number1>number1)
print(number1>=number2)
print(number1<number1)
print(number1<=number2)
print(number1==number2)
print(number1!=number1)

# 9

number1 = int(input("type any number: "))
number2 = int(input("type another number: "))

number11 = number1+10
number22 = number2+10

print(number1<number11)
print(number1<=number11)
print(number1>number11)
print(number1>=number11)
print(number1==number11)
print(number1!=number11)


# 10

number1 = int(input("type any number: "))
number2 = float(input("type another number: "))

number1_equal = type(number1) == int
number2_equal = type(number2) == float

number1_not_equal = type(number1) != int
number2_not_equal = type(number2) != float


print(number1_equal)
print(number2_equal)
print(number1_not_equal)
print(number2_not_equal)



















